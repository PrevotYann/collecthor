...
import FFTCGCard from "./FFTCGCard"; // Import the card component
...
const FFTCGCardsetsBrowser = () => {
  ...
  const fetchCardsets = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/cardsets/fftcg/all`
      );
  ...
  const handleSelectCardset = async (id, name) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/cardsets/fftcg/id/${id}/cards`
      );
  ...
  const handleBulkAdd = async () => {
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/items/table/cards_fftcg/items/user/${user.username}`,
        {
...
export default FFTCGCardsetsBrowser;
